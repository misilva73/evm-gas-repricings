...
4. **Binary tree + zk-friendly hash** 🤔. The long-pole, hardfork-gated change. It remains our best long-term tree design, but it is **not** a near-term state-project priority on its own merits. Whether we start now is **gated on the ZKVM team confirming it is a hard blocker for ZKVM's ship date** (or on privacy/AA needs such as new nullifier sets).
...
> **Note — this workstream is deferred and conditional.** We believe the binary tree is technically the right long-term design (small proofs, proving-friendliness, room for new state types). But from the state project's own perspective it is *not* the immediate priority. It would be the biggest fork change yet, larger than the merge, touching apps, wallets, RPCs, and many Solidity contracts. So we only commit to doing it **now** if it is a hard blocker for another team's ship date. The most likely trigger is **ZKVM**. The change the ZKVM team actually wants is the **hash function**, to reduce **proving time** (note: proof *size* of the ZKVM is independent of the tree's proof size). We need to extract from Justin, Ignasio, and Kev whether that is genuinely live-or-die for shipping. A second possible trigger is privacy/AA needing protocol-level new state types (e.g. nullifier sets). Until one of those is confirmed, MPT remains viable and we focus on the other five workstreams. Next step: a stakeholder meeting to settle necessity.
...
- Confirm binary-tree necessity 🤔: hold a stakeholder meeting (Justin, Ignasio, Kev, architecture team) to determine whether the migration is a hard blocker for ZKVM or privacy/AA. Proceed with spec/prototype work only if confirmed.
