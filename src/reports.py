...
def generate_runtime_report(
    ...
    md_file = MdUtils(
        file_name=os.path.join(
            out_dir, f"runtime_estimation_autogenerated_report"
        ),
...
"""
    ...
    for param in all_params:
        if param in results_df.columns:
            ...
            # Poor fits
            poor_fit_params_df = results_df[
                (results_df[param + "_pvalue"] >= 0.05) & (results_df["rsquared"] > 0.5)
            ][["opcode", "client"]].dropna()
            for row in poor_fit_params_df.itertuples():
    ...
    # Add poor fitted models
    poor_fit_model_df = results_df[(results_df["rsquared"] <= 0.5)][["opcode", "client"]].dropna()
    for row in poor_fit_model_df.itertuples():
        if (row.opcode, 'Model') not in poor_fit_dict:
            poor_fit_dict[(row.opcode, "Model")] = set([row.client])
        else:
            poor_fit_dict[(row.opcode, "Model")] = poor_fit_dict[
                (row.opcode, "Model")
            ].union(set([row.client]))
    # Save cost estimation
    new_gas_df.to_csv(
        os.path.join(out_dir, f"new_gas.csv"), index=False
    )
