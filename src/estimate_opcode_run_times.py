...
def process_gas_bench_data(user: str, password: str, start_date: str) -> pd.DataFrame:
    ...
    df["test_opcode"] = np.where(
        df["test_name"].isin(OPCPDES_IN_TEST_NAME_LIST),
...
def estimate_run_time_for_simple_opcode(
    ...
    for client in clients:
        ...
        try:
            ...
            plt.title(
                f"OLS Linear Regression for {opcode} opcode in {client} client\nIntercept: {intercept:.2f}, Slope: {slope:.2e}"
            )
            # Save plot
            fig.savefig(
